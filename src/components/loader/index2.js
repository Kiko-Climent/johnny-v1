...
const Splash2 = () => {
  ...
  useEffect(() => {
    ...
    setTimeout(() => el.classList.add("play"), 1000);    // title1 → title2
    setTimeout(() => el.classList.remove("play"), 3500); // title2 → title1

    // Animación del segundo div
    animateEmptyToTitle(el2, titles.title2);
    setTimeout(() => el2.classList.remove("play-2"), 1000);   // justo cuando el primero vuelve a title1
    setTimeout(() => el2.classList.add("play-2"), 3500);   // justo cuando el primero vuelve a title1
  }, []);
