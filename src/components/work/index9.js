  ...
  return (
    <div ref={body} className="flex flex-col h-screen w-screen relative">
      ...
      <Image
        ...
        style={{
          filter: hoveringProject ? "invert(1)" : "none",
      ...
      {!expanded ? (
        ...
        <>
          ...
          <div className="absolute top-[40%] w-full flex flex-col items-center justify-start z-20">
            <div className="text-white text-xl tracking-widest flex flex-col items-center gap-2">
              {projects.map((project) => (
                <Project
                  key={project.src}
                  project={project}
                  onHoverChange={setHoveringProject}
                />
              ))}
              
              <div onClick={() => router.push(`/work/${selectedElement.name.replace(/\s+/g, "-").toLowerCase()}`)} 
                  className="uppercase flex text-white text-[5vw] mt-4 cursor-pointer ">
                go to gallery
              </div>
            </div>
          </div>
